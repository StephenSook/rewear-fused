...
| 4.1 | Wire frontend to real artifacts + live endpoint | frontend, data | Stephen | ✅ | 3.3 | DONE (Jun 15): Vercel env `NEXT_PUBLIC_ENGINE_C_API_URL` set; deployed `/passport` shows "ENGINE C · LIVE · catboost-text-v1.0" reaching Vinh's Render endpoint; demo-safe fallback intact; Engine A/B still in-silico (swap at freeze) |
| 4.2 | Deploy | infra | all | ✅ | 1.3 | DONE (Jun 15): public prod LIVE at https://rewear-fused.vercel.app (clean `rewear-fused` project, `next build --webpack`); /passport 200 + PUBLIC + 0 console errors; Engine C live on Render. Warm the endpoint before demos (free-plan cold start), keepalive C3 pending, else the badge shows `cached` |
