...
- **Beat 2, Engine C, the working floor (20s)** [Passport]: "This is live. A compliance classifier on 631 real CPSC recall records routes every garment clear, lab-test, or divert against actual US and EU regulation, then emits a scannable passport. Scan it." [hand them the QR] "That is the piece Carter's deploys in year one."
...
- **Beat 4, Engine B, the shouldn't-be-possible moment (25s)** [Mol* live]: "What is rotating here is the reference serine-hydrolase scaffold, CALB, PDB 1TCA, the published anchor we designed against. It shows the exact Ser-His-Asp pocket the de novo enzyme reconstructs, engineered near-neutral pH so it does not damage the cotton. Our novelty target is no close structural match in the PDB, TM-score under 0.5; the panel shows the threshold we design to. The designed structure and its live PLACER and FoldSeek numbers drop in at our freeze gate, and the panel is labeled illustrative until then. No de novo carbamate hydrolase has ever been publicly disclosed. Download the reference scaffold to inspect the active-site geometry."
- **Beat 5, the close (20s)** [Loop]: "Here is the loop closing. The bond cleaves, the fiber depolymerizes into recoverable monomers, re-spun into a new onesie, the two colors fuse at the cleavage. Carter's already pays TerraCycle to downcycle. Engine C is a per-SKU compliance-and-passport subscription, priced as a fraction of the avoided SB 707 fee: we move that line item to a passport that lowers their EPR modulation and pre-positions the EU passport. Today's fee avoidance funds tomorrow's fiber loop. We're the Nexus Circular of textiles."
...
- **ALWAYS close with the fixed line, regardless of branch:** "Same line item, better outcome." Then branch on the judge:
  - **Sustainability judge:** "Even safe chemistry sheds microfibers; we don't hide it, we reframe it. Our end-of-life replaces an ocean-microplastic-shedding fate with controlled industrial hydrolysis. By our estimate, on the order of 2 tonnes of CO2-equivalent avoided per tonne diverted."
...
- **Q4, "Can AI really design a new block copolymer?"** "We DID the impressive thing on the enzyme side, that's a real de novo fold with no match in the PDB. On the fiber, we never claim generative invention: it's constraint-guided screening, we enumerate, score, and filter known chemistries, which is the honest and defensible method. Here's the funnel." [point at the funnel]
...
| ~2 t CO2e / tonne diverted | secondary LCA benchmark | ESTIMATE - say "by our estimate" |
