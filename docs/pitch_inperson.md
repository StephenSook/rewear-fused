...
- **Beat 4, Engine B, the engineered hydrolase (25s)** [Mol* live]: "Our swing was a de-novo carbamate hydrolase with no prior art; that GPU run we deferred. What we engineered and validated in-silico is the LigandMPNN-redesign fallback. What is rotating is the CALB serine-hydrolase scaffold, PDB 1TCA, the published template our lead design is a redesign of: it shows the exact Ser-His-Asp pocket, repositioned for carbamate hydrolysis at near-neutral pH so it spares the cotton. Real PLACER preorganization on the Lauko thresholds, FoldSeek TM about 0.8 to the template. Honestly a redesign, not a de-novo fold. Download the designed structure to inspect the active-site geometry."
- **Beat 5, the close, the shouldn't-be-possible moment (20s)** [Loop]: "Here is the loop closing. No one has co-designed a cleavable fiber and its matched enzyme together for textiles before; this is that loop, closing live on real data. The bond cleaves, the fiber depolymerizes into recoverable monomers, re-spun into a new onesie, the two colors fuse at the cleavage. By design the same material becomes a new onesie again and again, not pet bedding once and landfill forever: a regenerative loop, not a one-time downcycle. And because the fiber is drop-in to existing spinning, one license scales it without a single new factory. Carter's already pays TerraCycle to downcycle. Engine C is a per-SKU compliance-and-passport subscription, priced as a fraction of the avoided SB 707 fee: we move that line item to a passport that lowers their EPR modulation and pre-positions the EU passport. Today's fee avoidance funds tomorrow's fiber loop. We're the Nexus Circular of textiles."
...
- **Q4, "Can AI really design a new block copolymer?"** "Two honest answers. On the enzyme: our swing was a de-novo fold with no prior art, but that GPU run we deferred, so what we delivered is the LigandMPNN-redesign fallback, three real engineered hydrolases with repositioned Ser-His-Asp triads and real PLACER metrics, a redesign at TM about 0.8, not a de-novo fold. On the fiber we never claim generative invention: it's constraint-guided screening, we enumerate, score, and filter known chemistries. Both honest and defensible. Here's the funnel." [point at the funnel]
