...
## 4. GPU pre-flight (do this before any GPU spend)

- **Hardware:** RunPod **H100 / H200 / B200**. H200 (141 GB HBM3e, ~$4.39/GPU-hr as of May 2026 on RunPod) is the price/performance sweet spot for RFdiffusion2 + Boltz-2 inference; B200 (~$4.99-6.03/hr) if you want more headroom (Backend No Limits, hardware section).
...
- [ ] Engine B pipeline runs end-to-end on RunPod, smoke-tested before any production run, image pinned + second install path tested.
